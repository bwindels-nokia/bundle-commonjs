...
function fsResolver(encoding) {
    return function(replacements, normalizedName) {
        ...
        if(replacements && replacements.hasOwnProperty(normalizedName)) {
            ...
            fileName = require.resolve(normalizedName);
        }
...
function ModuleBuilder(resolver, findModuleNames) {
    ...
    this.resolver = resolver || fsResolver('utf8');
}
...
ModuleBuilder.prototype._processModule = function(item, callback) {
    ...
    try {
        moduleDescriptor = this.resolver(this.replacements, item.normalizedName);    
    } catch(err) {
    ...
    moduleDescriptor.read(function(sourceCode) {
        ...
        //and convert the module names to work items
        var childItems = this.findModuleNames(sourceCode)
            .map(ProcessModuleSet.item.bind(null, basePath));
