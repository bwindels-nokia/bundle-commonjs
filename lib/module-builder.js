...
var resolvers = require('./module-resolvers');
...
ProcessModuleSet.prototype.pop = function() {
...
function ModuleBuilder(resolver, findModuleNames) {
    ...
    this.resolver = resolver || resolvers.createFsResolver('utf8');
}
...
ModuleBuilder.prototype._processModule = function(item, callback) {
    ...
    moduleDescriptor.read(function(sourceCode) {
        ...
        var needsBasePath = resolvers.moduleNeedsBasePath(requiredModuleNames);
        if(needsBasePath) {
...
ModuleBuilder.prototype.addModule = function(moduleName, replacements, callback) {
    if(replacements) {
        this.replacementsResolver = resolvers.createReplacementsResolver(replacements);
    }
...
function partsInCommon(a, b) {
    ...
    var i, smallestLength = Math.min(aParts.length, bParts.length);
